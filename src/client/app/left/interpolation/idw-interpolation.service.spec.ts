...
export function main() {
  describe('IDW Interpolation Service', () => {
    ...
    it('should reset samples when required', () => {
      let interpolation = new IdwInterpolationService();
      let sampleLocation1 = new Point(0.1, 0.2);
      let sampleValue1 = 0.2;
      ...
      interpolation.resetSamples();

      expect(interpolation.valueAtLocation(sampleLocation1)).toBe(0);

      let nonSampleLocation = new Point(0.4, 0.7);
      expect(interpolation.valueAtLocation(nonSampleLocation)).toBe(0);
    });
  });
}
